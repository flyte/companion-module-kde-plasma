...
class KWinDbus {
  ...
  async connect() {
    try {
      ...
      this._onCurrentDesktopChanged = (n) => {
        if (this.onDesktopChanged) this.onDesktopChanged(Number(n))
      }
      this.kwin.on('currentDesktopChanged', this._onCurrentDesktopChanged)
    } catch (err) {
      this.disconnect()
      throw err
    }
  ...
  disconnect() {
    if (this.kwin && this._onCurrentDesktopChanged) {
      try { this.kwin.removeListener('currentDesktopChanged', this._onCurrentDesktopChanged) } catch (_) {}
    }
    this._onCurrentDesktopChanged = null
    if (this.bus) {
